...
export default {
  props: ['proposals', 'finalized', 'index'],

  setup(props) {
    const { proposals, finalized, index } = toRefs(props)
    const { childBlockHashes, latestProposal, oldestProposal } = index.value
    
    const nodes = vgraph.getNodes(proposals.value, latestProposal)
    const edges = vgraph.getEdges(proposals.value, latestProposal)
    const layouts = vgraph.getLayouts(proposals.value, {
      ...index.value,
      latestProposal,
    })
